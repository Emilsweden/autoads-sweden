...
function kundkort() {
  ...
  return '<div class="kundkort">' +
    ...
    (b.far_andra ? '<button class="btn btn-ghost" id="dAndra">Redigera bokning</button>' : '') +
    '</div></div>';
}

/** Redigera bokning, i dörrpanelen. Tillbaka och klart landar på dörren igen. */
function visaAndraBokning() {
  const b = aktivBokning();
  const a = aktuell.adress;
  if (!b) return;
  redigeraBokning(b, { adress: a, panel: 'dorr', klar: () => oppna(a.id), tillbaka: () => oppna(a.id) });
}
